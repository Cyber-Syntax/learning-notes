...
class DownloadProgress(ABC):
    ...
    @abstractmethod
    async def start(self, total_files, total_size, total_drpms=0):
        pass

    @abstractmethod
    async def progress(self, payload, done):
        pass

    @abstractmethod
    async def end(self, payload, status, err_msg):
        pass
...
class AsyncMultiFileProgressMeter(DownloadProgress):
        ...
        async with self.lock:
            if file_id in self.files:
                ...
                if status == STATUS_OK:
                    ...
                    self.files[file_id]["status"] = self.STATUS_2_STR.get(
                        status, "unknown"
                    )
    ...
    def _update_display(self):
        ...
        for file_id, file_info in sorted(
            self.files.items(), key=lambda x: x[1]["display_name"]
        ):
    ...
    def _display_summary(self):
        ...
        overall_pct = (
            int(total_downloaded * 100 / self.total_size) if self.total_size > 0 else 0
        )

        # Count by status
        downloading = sum(
            1 for f in self.files.values() if f["status"] == "downloading"
        )
    ...
    async def download_files(self, payloads):
        ...
        await self.start(len(payloads), total_size)

        # Create download tasks
        download_tasks = [self._download_file(payload) for payload in payloads]
        ...
        if self.isatty:
            ...
            for file_info in sorted(
                self.files.values(), key=lambda x: x["display_name"]
            ):
                status = file_info["status"]
                ...
                if status == "completed":
                    ...
                    speed = (
                        file_info["payload"].download_size / elapsed
                        if elapsed > 0
                        else 0
                    )
                    self.message(
                        f"✓ {name} - {format_time(elapsed)} ({format_number(speed)}B/s)\n"
                    )
                elif status == "failed":
                    error = file_info.get("error", "Unknown error")
                    self.message(f"✗ {name} - FAILED: {error}\n")

    async def _download_file(self, payload):
        ...
        if random.random() > 0.15:  # 85% success rate
            ...
            await self.end(payload, STATUS_FAILED, random.choice(errors))
...
async def main():
    ...
    print("=" * 50)
    print("\n" * 2)  # Reserve space for progress display
    ...
    payloads = [
        ...
        FakePayload(
            1024 * 1024 * 35,
            "https://archlinux.org/iso/2023.08.01/archlinux-2023.08.01-x86_64.iso",
        ),
        FakePayload(
            1024 * 1024 * 15,
        ...
        FakePayload(
            1024 * 1024 * 18,
            "https://mirror.alpinelinux.org/alpine/v3.18/releases/x86_64/alpine-standard-3.18.3-x86_64.iso",
        ),
    ]
    ...
    start_time = time.time()
    await meter.download_files(payloads)
    end_time = time.time()
